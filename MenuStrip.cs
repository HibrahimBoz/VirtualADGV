...
namespace Zuby.ADGV
{
    ...
    internal partial class MenuStrip : ContextMenuStrip
    {
        ...
        private AdvancedDataGridView _parentGrid;
        ...
        public MenuStrip(AdvancedDataGridView parentGrid, Type dataType)
            : base()
        {
        ...
        {
            if (string.IsNullOrWhiteSpace(checkTextFilter.Text) || checkTextFilter.Text == "Ara")
            {
                checkTextFilter.Text = "Ara";
        ...
        private void StyleModernButton(Button btn, Color backColor, Color foreColor)
        {
            ...
            // Add subtle rounded effect simulation if possible (WinForms limitation)
            // But just clean flat is better than default
        }
        ...
        public void Show(Control control, int x, int y, string columnName)
        {
            ...
            if (_parentGrid != null && _parentGrid.FilterAsyncLoadingNeeded != null)
            {
                ...
                // Start async loading
                Task.Run(async () => await BuildNodesAsync(dataGridView, columnName));
            }
            else
            {
        ...
        private async Task BuildNodesAsync(DataGridView dataGridView, string columnName)
        {
            try
            {
                var filterString = _parentGrid.FilterString;
                var result = await _parentGrid.FilterAsyncLoadingNeeded.Invoke(dataGridView.Columns[columnName], filterString);
            ...
            catch (Exception ex)
            {
                ...
                {
                    _loadedNodes = new TreeNodeItemSelector[] { };
                    TreeNodeItemSelector errorNode = TreeNodeItemSelector.CreateNode("Hata: " + ex.Message, null, CheckState.Indeterminate, TreeNodeItemSelector.CustomNodeType.Default);
                    _loadedNodes = new TreeNodeItemSelector[] { errorNode };
                    ChecklistReloadNodes();
                }));
            }
        }
        ...
        public void CleanFilter()
        {
            ...
            for (int i = 2; i < customFilterLastFiltersListMenuItem.DropDownItems.Count - 1; i++)
            {
                var item = customFilterLastFiltersListMenuItem.DropDownItems[i] as ToolStripMenuItem;
                if (item != null)
                {
                    item.Checked = false;
                }
        ...
        private void SetCustomFilter(string filterstring, string viewfilterstring)
        {
        ...
        private void SetCustomFilter(int filtersMenuItemIndex)
        {
            ...
            var item = customFilterLastFiltersListMenuItem.DropDownItems[filtersMenuItemIndex] as ToolStripMenuItem;
            if (item == null || item.Tag == null) return;

            string filterstring = item.Tag.ToString();
            string viewfilterstring = item.Text;
            ...
            if (customFilterLastFiltersListMenuItem.DropDownItems.Count > 2)
            ...
            // Rebuild checkbox list with filtered values
            if (_parentGrid != null && !String.IsNullOrEmpty(_columnName))
            {
                // Get filtered values from the grid
                var filteredVals = GetValuesForFilter(_parentGrid, _columnName);
                // Rebuild the checkbox list with only filtered values
                BuildNodes(filteredVals, _parentGrid, _columnName);
            }
        }
        ...
        private void CustomFilterLastFilterMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var item = customFilterLastFiltersListMenuItem.DropDownItems[i] as ToolStripMenuItem;
                if (item != null && item.Tag != null && item.Text == menuitem.Text && item.Tag.ToString() == menuitem.Tag.ToString())
                {
        ...
        internal void SetDataType(Type dataType)
        {
            ...
            {
                customFilterLastFiltersListMenuItem.Text = AdvancedDataGridView.Translations[AdvancedDataGridView.TranslationKey.ADGVDateFilters.ToString()];
            ...
            {
                customFilterLastFiltersListMenuItem.Text = AdvancedDataGridView.Translations[AdvancedDataGridView.TranslationKey.ADGVNumberFilters.ToString()];
            ...
            {
                customFilterLastFiltersListMenuItem.Text = AdvancedDataGridView.Translations[AdvancedDataGridView.TranslationKey.ADGVTextFilters.ToString()];
            ...
            // Populate Submenu
            PopulateCustomFiltersSubmenu();
        }
        private void PopulateCustomFiltersSubmenu()
        {
            customFilterLastFiltersListMenuItem.DropDownItems.Clear();
            ...
            foreach (string f in filters)
            {
                ...
                item.Click += (s, e) => CustomFilterMenuItem_Click(f, e);
                customFilterLastFiltersListMenuItem.DropDownItems.Add(item);
            }

            customFilterLastFiltersListMenuItem.DropDownItems.Add(new ToolStripSeparator());
            var customItem = new ToolStripMenuItem(AdvancedDataGridView.Translations[AdvancedDataGridView.TranslationKey.ADGVAddCustomFilter.ToString()]);
            customItem.Click += (s, e) => CustomFilterMenuItem_Click(null, e);
            customFilterLastFiltersListMenuItem.DropDownItems.Add(customItem);
        }

        private void CustomFilterMenuItem_Click(object sender, EventArgs e)
        {
            CustomFilterMenuItem_Click(sender as string, e);
        }

        private void CustomFilterMenuItem_Click(string initialFilter, EventArgs e)
        {
            //ignore image nodes
            if (DataType == typeof(Bitmap))
                return;
            ...
            if (flt.ShowDialog() == DialogResult.OK)
            {
                ...
                SetCustomFilter(filterString, viewFilterString);
            }
        }
